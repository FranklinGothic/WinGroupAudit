...
class share_cli:
    ...
    def _select_shares(self):
        ...
        while True:
            ...
            try:
                index = int(share_choice) - 1  # Convert to 0-based index
                if 0 <= index < len(self.share_list):
                    share_choice = self.share_list[index]
...
class data_present_cli:
    ...
    def _print_case(self, special_distinction):
        ...
        share_data = self.all_data["Shares"]
        for share_object in share_data:
            share_name = share_object["sn"]
            print(f"\n{special_distinction}: {share_name}")
            print("-" * (len(share_name) + 8))

            outer_perms = share_object["pm"]
            for inner_perms in outer_perms:
                for gr_dict in inner_perms:
